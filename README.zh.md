...
**本包因此刻意不声明任何 `peerDependencies`**：声明 peer 会让 pnpm 往你的 profile 里**再装一份共享包副本**，而装在 profile
根部的副本会**遮蔽安装目录的版本** —— 这正是上面那次事故的完整机制（我的 peer 声明让 pnpm 自动装了
`dsh-credentials@0.1.0-rc.8`，它又遮蔽了安装目录的 `0.1.2-rc.1`）。插件在装载期不需要任何 harness 包在场，这份声明没有存在的理由。

> 给所有 DSH 插件作者的提醒：**profile 里的共享包副本会遮蔽安装目录的版本**，因此（a）顶层第三方 `import` 是可致命的写法，
> （b）不必要的 `peerDependencies` 会主动制造这种遮蔽。
